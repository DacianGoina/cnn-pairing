"""
Experimental setup when cantor transformation is applied only one time (i.e. halved the number of features)
"""

from main.cantor_utils import generate_pairs_instances, cantor_transform_one_unit
import itertools
...
from main.model_utils import get_confusion_matrix, create_and_train_model

# Better use colab for this
# Cell1: Prepare setup
rows_num_v = 28
cols_num_v = 28
...
all_combinations = list( itertools.product(seed_to_pairs, used_data_percentage_vals, split_seeds_vals, split_percentage_test_vals, split2_seeds_vals) )
...
new_cols_size_v = 14
normalize_v = True
quadradic_downscaling_v = (True, 1)

#test_title = "MNIST_CIFAR_CANTOR_ONE_TRANSFORMATION"
test_title = "MNIST_FASHION_CANTOR_ONE_TRANSFORMATION"
it = 0
...
for seed_to_pairs_val,  used_data_percentage_val, split_seeds_val, split_percentage_test_val, split2_seeds_val in all_combinations:
  ...
  # convert data
  x_train = cantor_transform_one_unit(x_train, seed_to_pairs_val[1], new_rows_size_v, new_cols_size_v, normalize_v, quadradic_downscaling_v)
  x_test = cantor_transform_one_unit(x_test, seed_to_pairs_val[1], new_rows_size_v, new_cols_size_v, normalize_v, quadradic_downscaling_v)

  # train model, get results
  model, training_time, metrics_res = create_and_train_model(x_train, x_test, y_train, y_test, new_rows_size_v, new_cols_size_v)
...
export_dict_as_json(final_dict,"MNIST-FASHION-CANTOR-ONE-TRANSFORMATION.json")